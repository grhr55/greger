  ...
  return (
    <div className="min-h-screen w-full py-12   text-zinc-100 ">
     ...
     <div className="mt-32 max-w-[1000px] mx-auto ">
        {/* ХЕДЕР С ГРАДИЕНТОМ И ТЕМПЕРАМЕНТОМ */}
        <div className="flex flex-col md:flex-row justify-between items-baseline mb-8 gap-4 border-b border-white/5 pb-6">
          <div>
            <span className="text-emerald-400 text-xs font-black uppercase tracking-[0.3em] block mb-1">RANKING ATUALIZADO</span>
        ...
        {/* ТАБЛИЦА С УЛУЧШЕННЫМ РАСПОЛОЖЕНИЕМ КОЛОНОК */}
        <div className="overflow-hidden rounded-[24px] border border-emerald-500/10 bg-[#060B14]/70 backdrop-blur-xl shadow-3xl overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/[0.02] border-b border-white/5">
                <th className="p-6 text-zinc-400 text-[10px] uppercase font-black tracking-widest w-[40%]">Plataforma</th>
                <th className="p-6 text-zinc-400 text-[10px] uppercase font-black tracking-widest text-center">Velocidade</th>
                <th className="p-6 text-yellow-400 text-[10px] uppercase font-black tracking-widest text-right">Crypto Mín</th>
                <th className="p-6 text-emerald-400 text-[10px] uppercase font-black tracking-widest text-right bg-emerald-500/[0.02]">⚡ Pix Mín</th>
              </tr>
            </thead>
            <tbody className="divide-y">
              {(() => {
                // Mapeia dinamicamente os dados do seu arquivo datapartnes para a tabela
                ...
                return data.sort((a,b) => a.min - b.min).map((row, i) => {
                  ...
                  return (
                    <tr key={i} className={`group transition-all duration-200 hover:bg-emerald-500/[0.03] ${isWinner ? "bg-gradient-to-r from-emerald-500/[0.02] to-transparent" : ""}`}>
                      ...
                      {/* КОЛОНКА 1: БРЕНД + БЕЙДЖ */}
                      <td className="p-6">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                          <span className={`font-black uppercase text-base tracking-wide ${isWinner ? 'text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-yellow-300' : 'text-white'}`}>
                            {row.name}
                          </span>
                          {isWinner && (
                            <span className="w-fit bg-gradient-to-r from-yellow-400 to-amber-500 text-black text-[8px] font-black px-2 py-0.5 rounded-md shadow-sm">
                              MELHOR OPÇÃO 🔥
                            </span>
                          )}
                        </div>
                      ...
                      {/* КОЛОНКА 2: СКОРОСТЬ (В ЦЕНТРЕ) */}
                      <td className="p-6 text-center">
                        <div className="inline-flex items-center gap-2 text-xs font-bold text-zinc-400 bg-white/[0.03] px-3 py-1 rounded-full">
                          <div className={`w-1.5 h-1.5 rounded-full ${isWinner ? "bg-emerald-400 animate-pulse" : "bg-zinc-600"}`} />
                          {row.speed}
                        </div>
                      </td>

                      {/* КОЛОНКА 3: КРИПТА (ПО ПРАВОМУ КРАЮ) */}
                      <td className="p-6 text-right font-bold text-sm tabular-nums text-zinc-400">
                        <span className="text-[10px] font-normal text-zinc-600 mr-1">R$</span>{row.cript}
                      </td>

                      {/* КОЛОНКА 4: ПИКС (ГЛАВНЫЙ АКЦЕНТ СПРАВА) */}
                      <td className={`p-6 text-right font-black text-base tabular-nums bg-emerald-500/[0.01] ${isWinner ? "text-emerald-400 text-lg" : "text-zinc-200"}`}>
                        <span className="text-xs font-bold text-emerald-500/70 mr-1">R$</span>{row.min}
                      </td>
        ...
        <div className="mt-8 bg-gradient-to-r border border-emerald-500/20 rounded-[28px] p-8 flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-72 h-72 bg-emerald-500/10 blur-[90px] pointer-events-none" />
          
          {/* Блок бренда */}
          <div className="flex items-center gap-5 w-full md:w-auto">
            <div className="bg-gradient-to-br from-yellow-400 to-amber-500 p-4 rounded-xl shadow-lg shrink-0">
              <Trophy size={28} className="text-black" />
            </div>
            <div>
              <p className="text-yellow-400 text-[10px] font-black uppercase tracking-[0.2em] mb-1">RECOMENDADO PELO EXPERT</p>
              <h2 className="text-2xl md:text-3xl font-black uppercase italic text-white leading-tight">
          ...
          {/* Блок цены */}
          <div className="flex flex-row md:flex-col items-center md:items-end justify-between w-full md:w-auto border-t border-white/5 md:border-none pt-4 md:pt-0">
            <p className="text-zinc-500 text-[10px] font-black uppercase tracking-widest md:mb-1">Depósito Mínimo</p>
            <div className="text-4xl font-black text-white flex items-baseline gap-1 tracking-tight">
              <span className="text-lg font-bold text-emerald-500">R$</span>
